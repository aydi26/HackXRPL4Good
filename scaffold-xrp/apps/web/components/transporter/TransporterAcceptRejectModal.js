...
export default function TransporterAcceptRejectModal({ offer, action, onClose, onConfirm, isLoading }) {
    ...
    >
      ...
      >
        <h2 className="text-2xl font-bold text-white mb-4">
          {isAccept ? "Accept Transport" : "Reject Transport"}
        </h2>
        
        <div className="mb-6">
          <p className="text-white/70 mb-2">
            {isAccept 
              ? "Are you sure you want to accept this transport mission?"
              : "Are you sure you want to reject this transport mission?"
            }
          </p>
          <div className="bg-white/5 border border-white/10 rounded-lg p-3">
            ...
            <p className="text-white/60 text-sm">
              {offer.weight} kg • {offer.distance || "N/A"} km
            </p>
            {offer.transportPrice && (
              <p className="text-white/60 text-sm">Price: {offer.transportPrice} XRP</p>
            )}
          </div>
        </div>
        ...
        {!isAccept && (
          <form onSubmit={handleSubmit} className="mb-6">
            ...
            <textarea
              ...
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none"
            />
          </form>
        )}
